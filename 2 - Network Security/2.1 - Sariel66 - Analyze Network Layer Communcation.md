...
![image](https://github.com/user-attachments/assets/25535258-c54a-498f-9863-e4d2680ffd9d)
...
The UDP protocol reveals that currenting the DNS server is currently unreachable, as shown by the network analysis, the ICMP 203.0.113.2 is providing a return error message saying that “UDP port 53 is unreachable.” As a result, the current issue is the DNS Server is not responding. 


## Explain Your Analysis of the Data and Provide at Least One Cause of the Incident

 1. The incident at 13:24 or 1:24pm
 2. Several customers contacted the company to report to the IT Team that they cannot gain access to the company website and were getting an error “destination port unreachable after waiting for it to load
 3. The network security team is currently investigating the issue to determine what that cause is using The team used TCPdump (network analyzer) to see the network traffic surrounding the website.
 4. Next task is to go through the log and determine whether or not the DNS Server is either down or if port 53 is being blocked by the firewall for some reason.
 5. After looking at the log is shows that there were 2 other attempts at connecting to the port 53 in quick succession one at 13:26:32 and the other at 13:28:32 suggesting that either the DNS was misconfigured, or Denial-of-Service attack (DoS Attack) might have been the source of the problem .
